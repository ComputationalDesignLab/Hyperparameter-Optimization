...
for sample in samples:
    ...
    os.system("python ../../bohb_workers.py --train_size {} --runs {} >> log.txt".format(sample, num_runs))
    os.chdir("../..")

    toc = time.time()
    times.append(toc-tic)
    
    print(f"Elapsed time for {sample} using BOHB : {toc-tic} seconds")

print(f"Elapsed time using BOHB : {times} seconds")
