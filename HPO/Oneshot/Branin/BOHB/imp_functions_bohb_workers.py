...
def run_hpbandster(x_train, y_train, x_cv, y_cv, iteration, min_b, max_b):
    ...
    # NS.start()
    # w = MyWorker(x_train=x_train, y_train=y_train, x_cv=x_cv, y_cv=y_cv, nameserver='127.0.0.1', run_id='bohb')
    # w.run(background=True)
    cs = get_configspace()
    
    # bohb = HyperBand(configspace=cs, run_id='bohb', nameserver='127.0.0.1', min_budget=min_b, max_budget=max_b)
...
if __name__ == "__main__":
    ...
    # Start a worker
    # print(f"Starting worker {args.worker}", flush=True)
